...
public class BookPSTMTDAOImpl implements BookDAO {
	...
	public int insert(Book book) {
		...
		String sql = "INSERT INTO tbl_book1(title,author) VALUES(?,?)";
		try {
	...
	public ArrayList<Book> select() {
		...
		try {
			...
			while(rs.next()) {
				books.add(new Book(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getTimestamp(4)));
			} 
	...
	public int update(Book book) {
		int result = 0;
		String sql = "UPDATE tbl_book1 SET title = ?, author = ? WHERE num = ?";
		conn = DBHelper.getConnection();
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, book.getTitle());
	...
	public int delete(int num) {
		...
		String sql = "DELETE FROM tbl_book1 WHERE num = ?";
		conn = DBHelper.getConnection();
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
