...
class Nation{
	...
	@Override
	public boolean equals(Object obj) {//나라이름이 같으면 같은 객체로 지정하는 메소드(재정의)
...
public class Explain2 {
	...
	a:	while(true) {
			...
			//방법 1
			if(store.contains(nation)) {  //contains는 equals 로 비교
			...
			for(int i=0; i<store.size(); i++) {
				...
				//if(na.getCountry().equals(nation.getCountry())) {   == 105라인
				if(nation.equals(na)) {
					System.out.println(country + "(은)는 이미 존재합니다.");
					continue a;
				}
	...
	private void quiz() {
		...
		if(answer.equals(capitalFromUser)) {
			...
			//맞힌 정보는 삭제
			store.remove(nation); //1 번째 방법
			store.remove(0);         //2 번째 방법
		}  else {
	...
	public static void main(String[] args) {
		...
		new Explain2().run(); // == Expain2 e = new Explain2();  ->   e.run();
