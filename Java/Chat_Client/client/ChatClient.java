...
public class ChatClient {
	...
	public void connect() throws Exception {
		...
		InputStreamReader reader = new InputStreamReader(is); //1바이트를 2바이트의 문자기반스트림으로 바꿔주는거임
		...
		OutputStreamWriter osw = new OutputStreamWriter(os); //1바이트를 2바이트의 문자기반스트림으로 바꿔주는거임
		...
		while(isRun) {
			//console에서 사용자가 서버에 전달할 데이터를 입력받음
